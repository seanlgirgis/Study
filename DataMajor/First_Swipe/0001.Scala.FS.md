...
This is where your existing PySpark knowledge becomes a superpower. The concepts are identical — you're just writing Scala syntax. Let's make that connection explicit at every step.
...
// Always import implicits — enables DataFrame magic
...
Nearly identical — just Scala syntax.
...
```scala
// Method 1 — from a case class (most common in Scala)
case class Transaction(
  id:       String,
  amount:   Double,
  category: String,
...
## Datasets — Scala's Secret Weapon over PySpark

This is where Scala Spark pulls ahead of PySpark. A **Dataset** is a typed DataFrame — you get compile-time type safety:

```scala
// DataFrame — untyped, like PySpark
val df = transactions  // DataFrame[Row]

// Dataset — typed, Scala only
...
```scala
// DataFrame — error shows at RUNTIME (bad)
df.filter(col("amountt") > 1000)  // typo — crashes when job runs

// Dataset — error shows at COMPILE TIME (good)
ds.filter(t => t.amountt > 1000)  // typo — caught before job even runs!
```

At Capital One scale — billions of records — catching errors at compile time instead of runtime saves hours of debugging.
...
// Reading Parquet — most common in production
val df = spark.read
...
Exactly the same — just Scala syntax.
...
// Left join — keeps all transactions even without customer match
...
Left join output — T004 kept with nulls:
```
+----+----------+------+-------------+--------+
|  id|customerId|amount|         name|    tier|
+----+----------+------+-------------+--------+
...
## A Complete Pipeline — Putting It All Together
...
This is a production-grade Spark job. Notice how everything we learned feeds into it — case classes, pattern matching, Option/Try concepts, higher-order functions, all working together.
