...
(All Channels)               (Topic Structure)               (All Systems)
...
└─────────────┘              │  P199[T004][T048][T092]  │                  │
                    ...
                    ▼                     ▼                  ▼             │
         ┌──────────────────┐  ┌──────────────────┐  ┌──────────────┐     │
         │ STREAM PROCESSOR │  │ STREAM PROCESSOR │  │   ARCHIVER   │     │
         │  (Flink/Spark)   │  │  (Flink/Spark)   │  │   (Kafka     │     │
         │  Fraud Detection │  │  Transaction     │  │   Connect)   │     │
         └────────┬─────────┘  │  Decisioning     │  └──────┬───────┘     │
                  │            └────────┬─────────┘         │             │
                  ▼                     ▼                    ▼             │
         ┌──────────────┐    ┌──────────────────┐    ┌──────────────┐     │
         │    TOPIC:    │    │     TOPIC:       │    │   S3/Data    │     │
         │ fraud-alerts │    │approved-txns     │    │    Lake      │     │
         │              │    │declined-txns     │    │  (Forever)   │     │
         └──────┬───────┘    └────────┬─────────┘    └──────────────┘     │
                │                     │                                    │
                ▼                     ▼                                    │
         ┌──────────────┐    ┌──────────────────┐                         │
         │  Alerting &  │    │  Downstream      │◄────────────────────────┘
         │  Blocking    │    │  Systems         │    Analytics Pipeline
         │  Service     │    │  (Reporting,     │    reads raw transactions
         └──────────────┘    │   ML Training)   │    directly
                             └──────────────────┘
...
║   └─────────────┘      └─────────────┘      └─────────────┘  ║
║                                                               ║
║   If entire AZ-1 goes down →                                  ║
║   Broker 2 or 3 becomes new Leader for P0 automatically ✅    ║
...
║  Key = customer_id                                            ║
║                                                               ║
║  Customer C001 → always → Partition 0                         ║
║  [T001: $500][T045: $1200][T089: $50,000] ← in order ✅       ║
║                                                               ║
║  Customer C002 → always → Partition 1                         ║
║  [T002: $200][T046: $800][T090: $300] ← in order ✅           ║
║                                                               ║
║  Why this matters for fraud detection:                        ║
║  "C001 made 10 transactions in 5 minutes" ← only possible     ║
║   if all C001 transactions are in the same partition          ║
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  min.insync.replicas    = 2      (write confirmed by 2 replicas)
  acks                   = all    (producer waits for all ISR)
  retention.ms           = 7days  (hot layer)
  partitions             = 200    (parallelism at scale)
  partition.key          = customer_id (ordering guarantee)
  archive                = S3     (cold layer, forever)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

![2026-02-12_21-59-34](2026-02-12_21-59-34.png)
