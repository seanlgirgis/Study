...
def add_gitkeep_to_empty_dirs(root_path: Path) -> int:
    ...
    Add .gitkeep to empty directories (idempotent: skips if exists).
    Returns count of .gitkeep files added.
    """
    count = 0
    for dir_path in root_path.rglob("*"):
        if not dir_path.is_dir():
            continue  # Traverse only folders
        if is_empty_dir(dir_path):
            gitkeep_path = dir_path / ".gitkeep"
            if not gitkeep_path.exists():
                gitkeep_path.touch()  # Create empty file
                count += 1
    return count
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Add .gitkeep to empty directories in a tree (idempotent)."
    )
    parser.add_argument(
        "--path",
        type=Path,
