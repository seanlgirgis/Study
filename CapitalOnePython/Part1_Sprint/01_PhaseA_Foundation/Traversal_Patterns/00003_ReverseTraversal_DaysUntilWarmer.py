...
class Solution:
    def daysUntilWarmer(self, temps: List[int]) -> List[int]:
        ...
        for i in range(n - 1, -1, -1):   # start at last index, go left
            
            j = i + 1                      # look at the next day first
            while j < n and temps[j] <= temps[i]:
                if ret[j] == 0:
                    break                  # j has no warmer day → dead end
                j += ret[j]             # jump using already computed answer
            if j < n and temps[j] > temps[i]:
                ret[i] = j - i          # distance to warmer day
            print(temps[i],i, ret)
        return ret
...
test_cases2: List[tuple[List[int], List[int]]] = [
    ([73, 74, 75, 71, 69, 72, 76, 73], [1, 1, 4, 2, 1, 1, 0, 0])
