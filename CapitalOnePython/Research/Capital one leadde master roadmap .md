...
### D.5 Recursion & Backtracking

#### D.5.1 Backtracking Template
```python
...
#### D.5.2 Permutations & Combinations
- **LeetCode 46** — Permutations
- **LeetCode 78** — Subsets
- **LeetCode 39** — Combination Sum
- **LeetCode 40** — Combination Sum II (with duplicates)

#### D.5.3 N-Queens & Constraint Problems
- Row/column/diagonal tracking with sets
- **LeetCode 51** — N-Queens

---

## 🐍 PHASE E: MODERN PYTHON MASTERY
### *Write code like a Senior Engineer, not a student. CodeSignal scores quality.*

---

### E.1 Modern Python 3.10+ Syntax (Latest Industry Standard)

#### E.1.1 Type Hints — PEP 585 & PEP 604 (Python 3.9+/3.10+)
```python
# OLD (still valid, but legacy)
from typing import List, Dict, Optional, Tuple
def foo(nums: List[int]) -> Dict[str, int]: ...

# MODERN (Python 3.9+ — use in CodeSignal)
def foo(nums: list[int]) -> dict[str, int]: ...

# Union types — Python 3.10+
def bar(x: int | str | None) -> list[float]: ...
```

#### E.1.2 Structural Pattern Matching — `match/case` (Python 3.10)
```python
match command:
    case "quit":
        return
...
#### E.1.3 Walrus Operator `:=` — PEP 572 (Python 3.8+)
```python
# Without walrus
data = fetch_data()
if data:
...
# With walrus (more Pythonic)
if data := fetch_data():
    process(data)

# In while loop (very useful for CodeSignal)
while chunk := file.read(8192):
    process(chunk)
```

#### E.1.4 f-Strings Advanced (Python 3.12)
```python
...
#### E.1.5 Dataclasses (Python 3.7+) — For Simulation Problems
```python
...
### E.2 Python Standard Library Weapons Arsenal

#### E.2.1 `collections` Module
```python
from collections import (
    Counter,        # Frequency counting
    defaultdict,    # Auto-initializing dict
    deque,          # O(1) appendleft/popleft — use instead of list for queues
    OrderedDict,    # Insertion-order dict (Python 3.7+ dict is ordered anyway)
    namedtuple,     # Lightweight immutable record
    ChainMap        # Merge dicts without copying
)

# Counter operations
c = Counter("aabbcc")
c.most_common(2)   # [('a', 2), ('b', 2)]
c + Counter("ab")  # Combine counts
c - Counter("a")   # Subtract counts

# defaultdict
adj = defaultdict(list)
adj[1].append(2)   # No KeyError

# deque (BFS queue — ALWAYS use this, not list)
q = deque([start])
q.append(x)        # O(1) right append
q.appendleft(x)    # O(1) left append
...
#### E.2.3 `bisect` Module (Binary Search on Sorted Lists)
```python
import bisect

# bisect_left: insertion point (leftmost where arr[i] >= x)
bisect.bisect_left(arr, x)

# bisect_right: insertion point (rightmost where arr[i] <= x)
bisect.bisect_right(arr, x)

# insort: insert maintaining sort order
bisect.insort(arr, x)
```

#### E.2.4 `itertools` Module
```python
import itertools

itertools.combinations(arr, r)    # nCr, no repeats
itertools.permutations(arr, r)    # nPr
itertools.product(arr, repeat=2)  # Cartesian product
itertools.accumulate(arr)         # Running sum / prefix sums
itertools.chain(a, b)             # Flatten iterables
itertools.groupby(sorted_arr, key=lambda x: x[0])  # Group consecutive equals
```

#### E.2.5 `functools` Module
```python
...
# Python 3.9+ cache (simpler, no maxsize)
@cache
def dp(i, j):
    ...
...
### E.3 Pythonic Idioms (Write Senior-Level Code)

#### E.3.1 Comprehensions
```python
# List comprehension
...
#### E.3.2 Enumerate, Zip, Map, Filter
```python
# enumerate — always prefer over range(len())
for i, val in enumerate(arr, start=1):  # start=1 optional
    ...
...
# zip_longest (from itertools) — pads shorter
from itertools import zip_longest
for a, b in zip_longest(arr1, arr2, fillvalue=0):
    ...

# Unpacking with zip (matrix transpose)
transposed = list(zip(*matrix))
```

#### E.3.3 Sorting Tricks
```python
...
#### E.3.4 Early Returns & Guard Clauses
```python
# BAD — deeply nested
def process(arr):
    if arr:
...
# GOOD — guard clauses
def process(arr: list[int]) -> int | None:
    if not arr:
        return None
    if len(arr) == 1:
        return arr[0]
...
#### E.3.5 Edge Case Handling Template
```python
...
## 🎯 PHASE F: CODESIGNAL EXECUTION STRATEGY
...
- Complete the [CodeSignal Practice Assessment](https://app.codesignal.com/arcade) — use General section
- Practice with the exact CodeSignal IDE (tabs vs spaces — there have been bugs reported!)
- Know your shortcuts: run code (`Ctrl+Enter`), submit, navigate between problems

#### F.1.2 Environment Readiness
- Chrome or Firefox (latest version)
- Wired internet preferred (at least 500 kb/s upload/download)
- Quiet room — no one can enter the camera frame
- Government-issued photo ID ready
- Close all unnecessary applications (no Slack, no browser tabs)
- Disable all notifications

#### F.1.3 Mental Model: The Q-Order Strategy ⭐
```
OPTIMAL ORDER (confirmed by passers):
Q1 (Easy)     → < 10 min  → DONE
Q2 (Med)      → < 15 min  → DONE
Q4 (Med-Hard) → < 20 min  → DONE or partial
Q3 (Matrix)   → Remaining time → Partial or full

WHY: Q3 is the time sink. Many passers skipped Q3 and still advanced.
```
...
| **Day 10** | Light review of weak areas | Rest — no new problems | Logistics: ID, camera, browser, room check |
